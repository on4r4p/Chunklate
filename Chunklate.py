...
def GetInfo(Chunk, data):
        ...
        try:
            ...
            if len(IHDR_Depht) > 0:
                if IHDR_Depht not in ["1", "2", "4", "8", "16"]:
                    ...
                    ToFix.append(
                        "Error:-IHDR Depht: Wrong bit depht (depht must be 1,2,4,8 or 16)"
                    )
            else:
                print(
                    "-Bit depht :"
            ...
            if len(IHDR_Color) > 0:
                ...
                if IHDR_Color == "3":
                    if IHDR_Depht not in ["1", "2", "4", "8"]:
                        ...
                        ToFix.append(
                            "Error:-IHDR Color 3: Wrong bit depht with IHDR Color type 3 (depht must be 1,2,4 or 8)"
                        )
            else:
                print(
                    "-IHDR Color %s "
    ...
    if Chunk == "PLTE":
        ...
        for i in range(0, PLTNbr, 6):
            ...
            try:
                ...
                ToFix.append(
                    "Error:Error PLTEG wrong value at " + str(i + 2) + ":" + str(e)
                )
            ...
            try:
                ...
                ToFix.append(
                    "Error:Error PLTEB wrong value at " + str(i + 4) + ":" + str(e)
                )
    ...
    if Chunk == "tRNS":
        ...
        if len(IHDR_Color) == 0:
            ...
            ToFix.append(
                "Error:IHDR Color Have to be either 0,2 or 3 when used with tRNS"
            )
        elif len(data) == 0:
            print(
                "-tRNS Chunk %s "
            ...
            ToFix.append(
                "Error:IHDR Color IHDR Color Have to be either 0,2 or 3 when used with tRNS"
            )
            ...
            if IHDR_Color == "3":
                if b"PLTE" not in Chunks_History and b"sPLT" not in Chunks_History:
                    ...
                    ToFix.append(
                        "Error:-PLTE Chunk or sPLT is missing.(tRNS must be used after one of them)"
                    )
                ...
                if b"PLTE" in Chunks_History:
                    if (
                        ...
                        ToFix.append(
                            "Error:-tRNS Alpha indexes palettes entries must not be superior to PLTE entries"
                        )
                ...
                if b"sPLT" in Chunks_History:
                    if (
                        ...
                        ToFix.append(
                            "Error:-tRNS Alpha indexes palettes entries must not be superior to sPLT entries"
                        )
...
def ChunkForcer(File, Chunk, OldCrc, DataOffset, ChunkLenght):
    ...
    if Bingo is True:
        ...
        return CheckPoint(
            "ChunkForcer",
            ["Error:Data has been corrupted"],
            newdatax_copy,
            DataOffset,
            DataOffset + (ChunkLenght * 2),
            "-Replacing Corrupted %s Data:\n%s\n-With:\n%s"
            % (Chunk.decode(), datax, newdatax_copy),
            Chunk.decode(),
        )
...
def FindMagic():
    ...
    if pos != -1:
        ...
        if DATAX.startswith(magic) is False:
            ...
            Zankentsu = DATAX[pos::]
            return CheckPoint(
                "FindMagic", ["Cutting at Magic"], Zankentsu, hex(int(pos / 2))
            )
...
def FindFuckingMagic():
    ...
    if BestBingoCount <= 2 and int(BestBingoScore) >= 14:
        ...
        return CheckPoint(
            "FindFuckingMagic", ["Cutting at Magic"], Odin, hex(int(pos / 2))
        )
...
def CheckChunkStory(lastchunk, mode):
    ...
    if mode == "TheGoodPlace":
        ...
        Candy(
            ...
            + Sample_Name
            + "\n "
            + str([i.decode() for i in Used_Chunks]),
            "good",
        )
        ...
        if lastchunk in OnlyOnce:
            if lastchunk in Excluded:
                print(
                    ...
                    % (
                        Candy("Color", "red", lastchunk.decode()),
            ...
            if lastchunk in Before_PLTE:
                ...
                if lastchunk in Excluded:
                    print(
                        ...
                        % (
                            Candy(
                                "Color", "red", lastchunk.decode() + " is missplaced"
                            ),
                            lastchunk.decode(),
                            Candy("Emoj", "bad"),
                        )
        ...
        if b"IDAT" in Used_Chunks:
            ...
            if lastchunk in Excluded:
                print(
                    ...
                    % (
                        Candy("Color", "red", lastchunk.decode() + " is missplaced"),
                        lastchunk.decode(),
                        Candy("Emoj", "bad"),
                    )
                )
        ...
        if b"IDAT" not in Used_Chunks:
            ...
            if lastchunk in Before_PLTE and b"IHDR" not in Used_Chunks:
                ...
                Candy(
                    ...
                    % (
                        Candy("Color", "green", lastchunk.decode()),
                        [i.decode() for i in Excluded],
                    ),
                    "bad",
                )
            ...
            if lastchunk in After_PLTE:
                ...
                Candy(
                    ...
                    % (lastchunk, [i.decode() for i in Excluded]),
                    "bad",
                )
            ...
            if int(IHDR_Color) == 3:
                ...
                if Warning is False:
                    Warning = True
                    ToFix.append(
                        "Error:-There is a chance that some Critical Palette chunks are missing."
                    )
                    Candy(
                        "Cowsay",
...
def BruteChunk(CType, LastCType, ChunkLen):
    ...
    if BestBingoCount <= 2 and int(BestBingoScore) >= 2:
        ...
        CheckPoint(
            Orig_CT,
            [
                "Error:Found Chunk[%s] has wrong name at offset: %s but changing %s bytes turn it into a valid Chunk name: %s"
                % (Orig_CT, CToffX, int(BestBingoScore) - len(Orig_CT), BestBingoName)
            ],
...
def CheckChunkName(ChunkType, ChunkLen, LastCType, next=None):
            ...
            if name == CType:
                ...
                if next == None:
                    ...
                    CheckPoint(
                        CType,
                        [
                            "Error:Found Chunk[%s] Wrong Chunk name at offset: %s"
                            % (Orig_CT, CToffX)
                        ],
                        CToffX,
                        CToffI,
    ...
    if next == None:
        ...
        if wow >= 3:
            Candy(
                "Cowsay",
                " Zlib put a conventional limit on buff size up to 8912 bytes and this one is pretty big :",
                "com",
            )
            ...
            if len(Bytes_History) > 0:  ##ToFIx##
                if Bytes_History.count(Bytes_History[0]) - 1 == len(Bytes_History) - 1:
                    if Bytes_History.count(Bytes_History[0]) != len(Bytes_History):
                        Candy(
                            "Cowsay",
                            " That doesnt mean there cannot be an IDAT chunk bigger than 8912Bytes!but since all previous IDAT chunks had the same length , it seems to me that's a little odd that this very one in particular is different from the others...Unless this is the Last IDAT.Anyway that is just a thought let's find it out .",
                            "com",
                        )
        else:
        ...
        CheckPoint(
            "CheckChunkName",
            [
                "Error:Found Chunk[%s] has Wrong Chunk name after Chunk[%s] at offset: %s"
                % (Orig_NC, LastCType, NCoffX)
            ],
            Orig_NC,
            ChunkLen,
            LastCType,
...
def Question(Chunk, ValidCrc):
    ...
    if ValidCrc is True:
        ...
        Candy(
            "Cowsay",
            "Do you wish to try to fix this regardless of CRC's validity ?",
            "com",
        )
        ...
        Candy(
            "Cowsay",
            "I can try to handle those errors.Do you wish to try to fix them ?",
            "com",
        )
...
def Checksum(Ctype, Cdata, Crc, next=None):
    ...
    if checksum == Crc:
        ...
        if Ctype in ERRORSFLAG:
            print(
                "-%s Chunks Errors left to be fixed. %s\n"
...
def Relics():
    ...
    if len(ArkOfCovenant) >= 1:
        ...
        if len(ArkOfCovenant) == 1:
            ...
            for nb1, (file, file_value) in enumerate(ArkOfCovenant.items()):
                Chunkname = "".join(
                    [chnk.decode() for chnk in ALLCHUNKS if chnk.decode() in errors]
                )
                for nb2, (errors, errors_values) in enumerate(file_value.items()):
                    if "Error:Wrong Crc" in errors:
                        ...
                        if nb1 == 0:
                            ChunkForcer(
                                FILE_Origin, Chunk, OldCrc, DataOffset, ChunkLenght
                            )
                            return ()
                        else:
                            pass
    ...
    for nb, key in enumerate(PandoraBox):
        ...
        if "Data has been corrupted" in str(key):
            ...
            if AnciCheck is not True:
                return SaveClone(
                    PandoraBox[key][chkd + "4"].hex(),
                    PandoraBox[key][chkd + "1"],
                    PandoraBox[key][chkd + "2"],
                    (
                        "-Replacing %s with %s at %s"
                        % (
                            PandoraBox[key][chkd + "3"],
   ...
   `----'"""
    ...
    for info in infos:
        if info.startswith("Error:") or info.startswith("libpng error:"):
            ...
            PandoraBox[
                str(mod_name_str) + "_Error_" + str(Fnum) + ":" + str(info)
            ] = TOOLS
        ...
        if mod_name == "LibpngCheck":
            if "libpng error:" in info:
                ...
                Candy(
                    "Cowsay",
                    "Good ! Libpng is happy everything is done here Later then!",
                    "good",
                )
                TheEnd()
        ...
        if mod_name == "CheckLength":
                ...
                if ToolKit[0] == "IEND" and int(ToolKit[1]) == 0:
                    for key in PandoraBox:
                        if "No NextChunk" in str(key):
                            Candy(
                                "Cowsay",
                                "Ah this one is a false positive im removing it ..",
                                "good",
                            )
            ...
            if info == "Found NextChunk":
                ...
                return CheckChunkName(
                    Raw_NextChunk, int(ToolKit[1], 16), ToolKit[0], True
                )
